...
class H5PEditorAjax
{
   ...
   */
    ...
    public function action($endpoint)
    {
      ...
      case H5PEditorEndpoints::SINGLE_LIBRARY:
        ...
        $library = call_user_func_array(
            array($this->editor, 'getLibraryData'),
            $args
        );
    ...
    private function fileUpload($contentId = null)
    {
        ...
        if ($file->validate()) {
            $file_id = $this->core->fs->saveFile($file, isset($contentId) ? $contentId : 0);
            $this->storage->markFileForCleanup($file_id, isset($contentId) ? $contentId : 0);
        }
        $file->printResult();
    }
